...
The current native view is `qbiotic-flow Runs`, backed by `RunsTreeDataProvider` and `GetRunHistoryService`. It lists runs for the first workspace folder and displays status as the item description. Selecting an item opens a read-only run details webview through `GetRunDetailsService`.
