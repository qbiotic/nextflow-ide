...
3. Phase 1 - Workspace Detection & Activation (depends on 2): implement the workspace detection adapter and extension activation strategy for multi-root workspaces on top of the existing Nx scaffold and enforced dependency boundaries. Detection, root selection, multiple entrypoints, profile discovery, first command activation, manual smoke fixture, and explicit view states are implemented.
...
- /Users/CAE9/nextflow-ide/docs/pending_decisions_nextflow_vscode_ide.md - historical decision inventory; current MVP contracts are captured in `docs/mvp_prd.md`, `docs/official_extension_contract.md`, and `docs/nfr_baseline.md`.
